...
Normally, the `parent/child` structure would not be needed for multi-module applications, however it is better to use it here, as it allows us to better organize all of the projects. This structure allows all of the child projects to make use of the plugins defined in the parent `pom.xml`, without having to define them again in the child `pom.xml` files.
