...
You will build a unit converter application that converts heights from centimeters into feet and inches. The application will request the user to enter a height value, in centimeters, and will then process the input using functions found in the JAR file to return the corresponding height in Imperial units.
...
To use a Java library in your web module, a dependency relationship must be added between the two modules.

As you might have noticed, each module has its own `pom.xml` file. This is because each module is treated as an independent project. You can rebuild, reuse, and reassemble every module on its own.
...
The [hotspot=dependency-jar file=0]`Java library module` and the [hotspot=dependency-war file=0]`web module` were added as dependencies. Specify [hotspot=warType file=0]`<type>war</type>` for the web module. If you don’t specify the web module, then Maven will look for a JAR file.
...
You must configure the `server.xml` to include the [hotspot=EARdefinition file=1]`enterpriseApplication` tag so that the server.xml knows to look for an EAR 
application, and where to look for it. 
