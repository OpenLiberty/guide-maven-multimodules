...
Set the [hotspot=packaging file=0]`basic configuration` for the project. Set `pom` as the [hotspot=packagingType file=0]`packaging` element of the parent [hotspot file=0]`pom.xml` file.
...
Add the [hotspot=liberty-maven-plugin file=0]`liberty-maven-plugin` plug-in to the parent pom.xml file.
This allows each child module to inherit the plug-in, so that you can use the Liberty Maven plug-in to develop the modules.
