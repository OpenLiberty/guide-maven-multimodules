...
It is worth noting that the [hotspot=parent file=0]`parent/child` structure is not needed for multi-module projects, but is used here to avoid using the fully qualified plugin name when running the dev mode command later on in the guide.
...
Update the `Converter` class for ...
