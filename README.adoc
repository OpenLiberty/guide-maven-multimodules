...
You will learn how to establish a dependency between a web module and a Java library module. You will use Maven to package the WAR file and the JAR file into an EAR file so that you can run and test the application on Open Liberty. You'll learn how to use the Liberty Maven Plugin to develop a multimodules application in development mode, without having to prebuild the JAR and WAR files, as your changes are automatically picked up.
...
To deploy your EAR application on an Open Liberty server, run the Maven `liberty:run` goal from the `finish` directory using the `-pl ear` flag:
