...
Next, add the dependencies. Define the [hotspot=26-31 file=0]`Java library module` and the [hotspot=32-37 file=0]`web module` as dependencies. Specify [hotspot=31]`<type>war</type>` for the web module. If you don’t specify the web module, Maven looks for a JAR file.

Add the definition and configuration of the [hotspot=50-70 file=0]`maven-ear-plugin` plug-in for creating an EAR file. Define the [hotspot=56-60]`<jarModule>` and [hotspot=61-67 file=0]`<webModule>` modules to be packaged into the EAR file.
...
To download and start an Open Liberty server, use the [hotspot=74-104 file=0]`liberty-maven-plugin` plug-in for Maven. This configuration is provided, and the executions of the plug-in follow the typical phases of a Maven life cycle.
...
[role="code_command hotspot=1-100", subs="quotes"]
----
Edit the POM file.
`start/pom.xml`
----
...
----
Set the basic configuration for the project. Set `pom` as the [hotspot=17 file=0]`<packaging />` element of the parent `pom.xml` file. Specify `io.openliberty.guides` as the [hotspot=14 file=0]`<groupId />` ID, which the child `pom.xml` files will inherit.
