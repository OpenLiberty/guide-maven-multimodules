...
Next, add the dependencies. Define the [hotspot=16-21 file=0]`Java library module` and the [hotspot=22-27 file=0]`web module` as dependencies. Specify [hotspot=26 file=0]`<type>war</type>` for the web module. If you don’t specify the web module, Maven looks for a JAR file.

Add the definition and configuration of the [hotspot=38-58 file=0]`maven-ear-plugin` plug-in for creating an EAR file. Define the [hotspot=44-48 file=0]`<jarModule>` and [hotspot=49-55 file=0]`<webModule>` modules to be packaged into the EAR file.
...
For a Maven EAR project, the [hotspot=106 file=1]`testCompile` goal is specified for the [hotspot=98-110 file=1]`maven-compiler-plugin` plug-in in your [hotspot=1-63 file=1]`ear/pom.xml` file so that the test cases are compiled and picked up for execution.
...
----
#Add a `&ltparent/&gt;` section just above `basic configuration` to each of the `pom.xml` files in the child modules,
