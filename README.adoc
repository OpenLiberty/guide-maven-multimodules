...
Navigate to the `start/ear/src/test/java/it/io/openliberty/guides/multimodules` directory. Open the [hotspot file=0]`Test.java` file. Helper functions for the integration test class are provided.
...
For a Maven EAR project, the [hotspot=106 file=1]`testCompile` goal is specified for the [hotspot=98-110 file=1]`maven-compiler-plugin` plug-in in your [hotspot file=1]`ear/pom.xml` file so that the test cases are compiled and picked up for execution.
