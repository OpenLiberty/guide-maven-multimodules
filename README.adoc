...
You will learn how to establish a dependency between a web module and a Java library module. You will use Maven to package the WAR file and the JAR file into an EAR file so that you can run and test the application on Open Liberty. You'll learn how to use the Liberty Maven Plugin to develop a multimodules application in dev mode, without having to prebuild the JAR and WAR files, as your changes are automatically picked up.
...
To deploy your EAR application on an Open Liberty server, run the Maven `liberty:run` goal from the `start` directory using the `-pl ear` flag:
...
It is worth noting that the [hotspot=parent file=0]`parent/child` structure is generally not needed for multi-module projects, but is required here to better organize all of the projects.
...
Add the [hotspot=liberty-maven-plugin file=0]`Liberty Maven Plugin` to the parent `pom.xml` file. This allows each child module to inherit the plugin, which allows you to install applications into Open Liberty.
...
Now you can develop the application and the different modules together using the Liberty Maven Plugin developer mode.
...
When you are done checking out the service, exit dev mode by pressing `CTRL+C` in the command-line session where you ran the server, 
...
You have aggregated and developed the application. Now, you can run `mvn install` once from the `start` directory and it will automatically build all your modules. This command creates a JAR file in the `jar/target` directory, a WAR file in the `war/target` directory, and an EAR file in the `ear/target` directory, which contains the JAR and WAR files.
...
Alternatively, you can run the `liberty:stop` goal from the `start\ear` directory in another command-line session:
...
For a Maven EAR project, the [hotspot=testCompile file=0]`testCompile` goal is
specified for the [hotspot=maven-compiler-plugin file=0]`maven-compiler-plugin`
plug-in in your [hotspot file=0]`ear/pom.xml` file so that the test cases are
