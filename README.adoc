...
:page-seo-description: A getting started tutorial and an example on how to use Maven to build a Java or Java EE application with multiple modules consisting of Java archive (JAR), web archive (WAR), and enterprise archive (EAR) projects.
...
You will learn how to establish a dependency between a web module and a Java library module. You will use Maven to package the WAR file and the JAR file into an EAR file so that you can run and test the application on Open Liberty. You'll learn how to use the Liberty Maven Plugin to develop a multi-module application in development mode, without having to prebuild the JAR and WAR files, as your changes are automatically picked up.
...
To deploy your EAR application on an Open Liberty server, run the Maven `liberty:run` goal from the `finish` directory using the `-pl ear` or `--projects ear` flag:
...
Normally, the [hotspot=parent file=0]`parent/child` structure would not be needed for multi-module applications, however it is better to use it here, as it allows us to better organize all of the projects. This structure allows all of the child projects to make use of the plugins defined in the parent `pom.xml`, without having to define them again in the child `pom.xml` files.
...
Now you can develop the application and the different modules together using the Liberty Maven plug-in development mode.
To learn more about to use development mode with multiple modules, check out the link:https://github.com/OpenLiberty/ci.maven/blob/main/docs/dev.md#multiple-modules[Documentation].
