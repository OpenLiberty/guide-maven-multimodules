...
----
Go to the dependency section, where you can add more dependencies. Add the [hotspot=39-43 file=0]`<dependency />` element: the Java library module that implements the functions that you need for the unit converter.
...
Add the [hotspot=36 file=1]`getFeet(cm)` to the [hotspot=33-40 file=1]`setHeightFeet` method to convert a measurement into feet.

Then, add [hotspot=47 file=1]`getInches(cm)` to the [hotspot=42-49 file=1]`setHeightInches` method to convert a measurement into inches.
...
Set the [hotspot=7-11]`basic configuration` for the project and set the packaging type to [hotspot=11]`ear`.

Next, add the dependencies. Define the [hotspot=26-31]`Java library module` and the [hotspot=32-37]`web module` as dependencies. Specify [hotspot=31]`<type>war</type>` for the web module. If you don’t specify the web module, Maven looks for a JAR file.

Add the definition and configuration of the [hotspot=50-70]`maven-ear-plugin` plug-in for creating an EAR file. Define the [hotspot=56-60]`<jarModule>` and [hotspot=61-67]`<webModule>` modules to be packaged into the EAR file.
...
To download and start an Open Liberty server, use the [hotspot=74-104]`liberty-maven-plugin` plug-in for Maven. This configuration is provided, and the executions of the plug-in follow the typical phases of a Maven life cycle.
...
Add the [hotspot=18-24 file=0]`testIndexPage` test to check that you can access the landing page.

Add the [hotspot=29-34 file=0]`testHeightsPage` test to check that the application can process the input value and calculate the result correctly.

For a Maven EAR project, the [hotspot=116 file=1]`testCompile` goal is specified for the [hotspot=108-120 file=1]`maven-compiler-plugin` plug-in in your `ear/pom.xml` file so that the test cases are compiled and picked up for execution.
...
----
Set the basic configuration for the project. Set `pom` as the [hotspot=18 file=0]`<packaging />` element of the parent `pom.xml` file. Specify `io.openliberty.guides` as the [hotspot=15 file=0]`<groupId />` ID, which the child `pom.xml` files will inherit.

In the parent `pom.xml` file, list all of the [hotspot=22-26 file=0]`<modules />` that you want to aggregate for the application.

Lastly,
[role="code_command hotspot=16-20", subs="quotes"]
----
Add a `<parent />` section to each of the `pom.xml` files in the child modules,
