...
Add a [hotspot=Config file=1]`<parent/>` section just above `basic configuration` to each of the `pom.xml` files in the child modules,
such as the [hotspot=parent-jar file=2]`jar`, [hotspot=parent-war file=3]`war`, and [hotspot=parent-ear file=4]`ear` projects.

pom.xml
[source, xml, linenums, role='code_column tags=Config']
----
include::finish/ear/pom.xml[]
----
