...
You will build a unit converter application that converts heights from centimeters into feet and inches. The application will request the user to enter a height value, in centimeters, and will then process the input within functions found in the JAR file to return the corresponding height in Imperial units.
