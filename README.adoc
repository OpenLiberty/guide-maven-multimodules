...
For a Maven EAR project, the [hotspot=testCompile file=1]`testCompile` goal is specified for the [hotspot=maven-compiler-plugin file=1]`maven-compiler-plugin` plug-in in your [hotspot file=1]`ear/pom.xml` file so that the test cases are compiled and picked up for execution.
...
Enter the following command to start the server, run the test from the `start/ear` directory, then stop the server:
